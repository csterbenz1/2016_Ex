...
```{r setup_params}
## Erin's path
#path_data = "~/Dropbox/Documents/2019__2020/work/kpop/application/data/"
#Ciara's 
...
#XXXXX 360 still NA how are thee being dealt with?
sum(is.na(cces$recode_vote_2016))
## make recode_educ_white column
#what do we need this for? educ_3way does this I think but only 3 levles not all 6
cces <- cces %>%
...
## XXXXXX Fixed as of July 2020 (model missing age in data_prep file)
## recode missing age
...
#failed to converge XXXX can add epsilon = 0.005 to match sims to get rid of this
...
CJH changing this one to drop recodeage -- I'm not wild about having it when we already have the bucketed age, but also because the mean imputation it has a weird distribution, and kbal care about the distribution.

```{r kpop, eval = T, warning= F }
# XXXX unlike sims we do not include the white x education thing directly
kpop_data <- rbind(pew %>% select(recode_age
   ...
   model.matrix(as.formula("~."), .)  # CJH: putting intercept back in so colinear columns drop.

kpop_data =kpop_data[,-1] # remove the actual intercept.

kpop_sampled <- c(rep(1, nrow(pew)), rep(0, nrow(cces)))


kpop_b2x <- kbal(allx=kpop_data,
                    sampled = kpop_sampled,
                    b = 2 * ncol(kpop_data),
                    fullSVD = TRUE,
                    meanfirst = FALSE,
                    incrementby = 5,  # speed it up a bit with 5 here
...
```{r save_plot}
ggsave("./plots/weighted_pew_results_chad.pdf", width = 6, height = 4)
```
...
```{r}
test <- bind_rows(cces %>% select(recode_age) %>% mutate(data = "cces")
                  , pew %>% select(recode_age) %>% mutate(data = "pew_with_recode")
                  , pew %>% filter(!str_detect(age, "Refused")) %>% select(recode_age) %>% mutate(data = "pew_without_recode"))

ggplot(test, aes(fill = data, x = recode_age)) + geom_density(alpha = 0.4)
```
