...
\section{Simulations Code}
\label{app:simulations}

In Section \ref{app:simulation} we briefly described our simulation study.  In this
section of the supplementary materials we describe the simulations in more detail
and provide replication code.
...
#this has bad kpop coverage, also used reduced kbal data so ng
...
Looking visually at the selection probabilities by party id, we see clear non-linearity, with a favoratism toward republican and indepdendent voters

```{r, echo = F, include = F, fig.width=10, fig.height=8}
gg_p_include_pid
```
...
This yields a population target in percentage points of $\bar{Y} =49.14\%$. The correlation between selection probability and probability of voting democratic is $\approx -0.6$
...
- **"post_stratification"** applies ps on the true model (pid, education (3way), gender, age (4way), race, bornagain)
...
#plot_lasso_margin_r5$estimator_name = gsub(" aMF", "+MF", plot_lasso_margin_r5$estimator_name)
gg_out_r5_paper =  ggplot(data = plot_lasso_margin_r5 %>%
                              filter(!(estimator %in% c("kpop_conv",
                                                      "hayek_truth",
                                                      "ht_truth",
...
#plots MSE: bias^2 + var
# table_r5_paper = table_r5_paper %>% mutate(Estimator = rownames(table_r5_paper))
# ggplot(table_r5_paper) + 
#     geom_point(aes(x = Estimator, y = MSE)) + 
#     theme_bw() + 
...
rnames = rownames(table_r5_paper)
rnames[2] = "Post-Stratification"
rnames[6] = "kpop+MF (All)"
rownames(table_r5_paper) = rnames
table_r5_paper = table_r5_paper %>% arrange(-MSE)
kable(table_r5_paper, format = "latex", booktabs = T, digits = 3,
      caption = paste("Simulation Results across", length(good), "simulations (arranged by MSE)"),
...
sim_margins <- function(sims_file, pop_weighted_avg = TRUE) {
    ...
    if(colnames(margins)[1] != "margins.sample") {
    ...
    margins_base <- data.frame(
      ...
      post_strat_all = margins[,grepl("margins.post_strat_all",
                                        colnames(margins))] %>% rowMeans(),
      rake_truth = margins[,grepl("margins.rake_truth", colnames(margins))] %>% rowMeans())
    ...
    truth_aes <-  sapply(wmabserr_nooutcomes$var, function(x) {
            ifelse(grepl(x, as.character(selection_model)[2]) | x %in% c("agesq", "agecubed"), "gray", "black")
        })
    ...
    kable_margins <- kable(kable_prep[-c((nrow(kable_prep)-1):nrow(kable_prep)),],
                        ...
                        column_spec(which(colnames(wmabserr_nooutcomes) == "post_strat_reduc"),
                                                  color =ps_reduc_aes[-c((nrow(kable_prep)-1):nrow(kable_prep))])
...
gg_out_r5_paper
ggsave("/Users/Ciara_1/Dropbox/kpop/2023/sims/plots/sims_paper_methods_boxplot_v2.pdf",
      width = 8.5, height = 4.5)
...
Distribution of our 1000 SE estimates by weighting method and SE estimator:
...
The following table shows the average moments of the weights by method across 1000 simulations. Note that \textbf{"Effective SS"} refers to the effective sample size, but is not well referenced against a set sample size since we this varied across simulutions because we used bernoulli draws. The average sample size should be $\sum(p(S=1)) = 513.1844$ and which empirically was on average 512.337. \textbf{"sum 90perc"} is the number of units required to sum to $90%$ of the total sum of the weights when weights are ordered from largest to smallest. In other words, summing from the largest weights to the smallest, we require this number of units to get to $90%$ of the totall sum of the weights.
...
What about the dimensions of K that are balanced on? We see, that the simulations with larger SEs balance on fewer dimensions of K on average, and have a lower variance in this selection. This makes sense as weights that balance on a greater number of dimensions of U account for more variance in the outcome yielding smaller SEs.
