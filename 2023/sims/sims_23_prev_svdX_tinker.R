...
check_sample_outcome <- function(sample, pop = cces, selection_model, interaction_cols, interaction_cols_2 = NULL) {
    ...
    run_counts <- function(data) {
        for(i in 1:length(vars)) {
            t = data %>% group_by_at(vars[i]) %>%
                ...
                mutate(prop = round(n/nrow(data),4))
    ...
    samp_counts$pop_prop = pop_counts$prop
    samp_counts$diff = samp_counts$prop - samp_counts$pop_prop
    samp_counts$diff_10 = abs( samp_counts$diff) >= .1
...
interaction_cols_2 = c("recode_born", "recode_age_bucket")
interaction_cols = c("recode_female", "recode_pid_3way")
inter = interaction_cols
inter_2 = interaction_cols_2
...
# coefs[,1] = c(-2, #dem x 18-36 x not born
#               2, #ind (x 18-36)
#               2, #rep (x 16-36)
#               .5, #male
#               -1, #36-50 (x not Born)
...
#               2 #rep x 65+
# )
# coefs[,1] = c(-3, #female dem not born 
#               2, #ind x female
#               1, #rep x female
#               1, #male (dem) #RELATIVE TO INTERCEPT (so more male dems than female dems)
#               -5, #36-50 (x  no born)
#               -5, #51-64 (x  no born)
#               -6, #65+, (x  no born)
#               .7, #college
...
#               3 #rep x male
# )
#only .3% bias for rake all
coefs[,1] = c(-1, #female dem not born 
              1, #ind x female
              2, #rep x female
              1, #male (dem) #RELATIVE TO INTERCEPT (so more male dems than female dems)
              -2, #36-50 (x  no born)
...
sum(p_include)
div = sum(p_include)/500
#pS_denom = 32
p_include = p_include/div
sum(p_include)
...
#coefs_outcome[1] = 19
#coefs_outcome[1] = 17
#for coefs/2: coefs_outcome[1] = 9
#coefs_outcome[1] = 11 #5/8
#coefs_outcome[1] = 11#11 #6/10
coefs_outcome[1] = 17
cor(p_include, cces_expanded %*% coefs_outcome)
...
#raking on the truth breaks here because we have weights w zero for born x 65+
#so i'll adjust that coef and try agian

# svymean(~recode_pid_3way, rake_truth_svyd, estimate.only = T )
# 
# sum(rake_truth_svyd$variables$outcome*rake_truth_svyd$postStrata[[1]]$w)
# 
# #error is here:
# qr.resid(rake_truth_svyd$postStrata[[1]]$qr, 
#          rake_truth_svyd$variables$outcome/rake_truth_svyd$postStrata[[1]]$w)
# #look at weights
# rake_truth_svyd$variables$outcome/rake_truth_svyd$postStrata[[1]]$w
# #ohh do we have zero weights?
# sum(rake_truth_svyd$postStrata[[1]]$w == 0)
# #oh shit wait how
# weights = rake_truth_svyd$postStrata[[1]]$w 
# weights[which(rake_truth_svyd$postStrata[[1]]$w  == 0)]
# #looks like 65+ year olds born agains are the issue
# #the coef on them is just too big I think
# survey_sim[which(rake_truth_svyd$postStrata[[1]]$w ==0),] %>% select(all.vars(selection_model))
# check = rake_truth_svyd$postStrata[[1]]$w
# 
# length(check)
# sum(rake_truth_svyd$variables$outcome*rake_truth_svyd$postStrata[[1]]$w)
# cbind(weights(rake_truth_svyd), check)
# 
...
system.time({
  sims <- mclapply(1:nsims, function(nsim) {
    ...
    check_2 = check_sample_outcome(survey_sim, pop = cces, 
                                   selection_model, 
                                   interaction_cols = inter, interaction_cols_2 = inter_2)
    ...
    if(class(rake_demos_weduc_svyd)[1] == "try-error") {
        ...
        # res_rake_demos_wedu = data.frame(min = NA, 
        #                                  perc_25 = NA, 
        #                                  mean = NA,
        #                                  perc_75 = NA,
        #                                  var = NA)
    ...
    if(class(rake_all_svyd)[1] == "try-error") {
        ...
        res_rake_all <- data.frame(min = NA,
                                   perc_25 = NA, 
                                   mean = NA, 
                                   perc_75 = NA,
                                   var = NA) 
...
sims[-good]
#sims = out
#raking on demos w educ seems to have broken here and there
savesims = sims

if(SAVE) {
    save(sims, outcome, tolerance, maxit, increment, min_num_dims, max_num_dims, noise,
         R2_outcome, eval_kpop,
         coefs, coefs_outcome, selection_model, p_include, pS_denom,
         file = paste0("./res_kpop", eval_kpop, 
                       "_noise", noise, "_on",
                       Sys.Date(),
                       "_nsims", length(good),
                       ".RData"))
...
########## SEs
SE_coverage = all_SE_coverage(sims, truth = mean(outcome), drop_NA = T, adjust_bias = T)
SE_coverage

SEs = lapply(sims, `[[`, 2) %>% bind_rows()
est <- lapply(sims, `[[`, 1) %>% bind_rows()
cols = if(eval_kpop) { c(3:10,12:16)} else {c(3:ncol(est))}
...
#on average sample has 3/24 cats important in the selection model that have <5% units in it
mean(samp_check$check.leq_5pp)
#on average sample has no important in the selection model that have <1% units in it
...
if(SAVE) {
    save(sims, outcome, SE_coverage, bias, table, plot_lasso_margin, noise,eval_kpop,emp_SE,
         ...
         coefs, coefs_outcome, selection_model, p_include, pS_denom,
         file = paste0("./eval_res_kpop_", eval_kpop,
                       "_noise", noise, "_on",
                       Sys.Date(), 
                       "_nsims", length(good),
                       ".RData"))
