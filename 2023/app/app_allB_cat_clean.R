...
save(cat_app, tolerance, maxit,
     file = paste0("allB_nodiag_cat_lasso061021_w", POPW, "_m",maxit, "_t",tolerance, "_",
                   Sys.Date(), ".Rdata"))
out_cat <-lapply(lapply(cat_app, `[`, 2), '[[', 1)
out_cat <- out_cat %>% bind_rows()
weights_cat <- lapply(lapply(cat_app, `[`, 1), '[[', 1)
save(out_cat, weights_cat, tolerance, file = paste0("allB_nodiag_cat_lasso061021_w", POPW, "_m",maxit, "_t",tolerance, "_",
                                                    Sys.Date(), ".Rdata") )
print(out_cat)