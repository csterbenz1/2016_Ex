...
if(!coverage_eval) {
    ...
    if(linear_model) {
        ...
        if(min(xbeta_outcome) <0) {warning("outcome is less than 0 for some units when noise is added", immediate. = T)}
        ...
        coefs_outcome[,1] = c(5, #intercept
                                          -.05, #selection of indep pos
                                          -.08, #selection of R pos
                                          -.08, #36-50,
                                          .01, #51-64,
                                          -.2, #65+,
                                          .3, #male #.4 for male only seleciotn model
                                          -1, #hisp
                                          -.7, #other
                                          -.5, #white
                                          2.8,#ind x 36-50
        ...
        # cor(xbeta_outcome, p_include)
        cces[which(xbeta_outcome == min(xbeta_outcome)),
             c("recode_pid_3way", "recode_age_bucket", "recode_female", "recode_race")]
        cces[which(xbeta_outcome == max(xbeta_outcome)),
             c("recode_pid_3way", "recode_age_bucket")]
