...
est_mean("diff_cces_on_cces", kpop_svyd)
kpop_se
...
rake_demos_weduc_se
est_mean("diff_cces_on_cces", rake_demos_weduc_svyd)
...
sum(weights(rake_demos_weduc_svyd))
var_fixed(Y = rake_demos_weduc_svyd$variables$diff_cces_on_cces, 
          weights = weights(rake_demos_weduc_svyd),
          pop_size = nrow(cces))
...
out = var_fixed(Y = rake_demos_weduc_svyd$variables$diff_cces_on_cces, 
                weights = weights(rake_demos_weduc_svyd)*nrow(pew),
                pop_size = nrow(cces))
out
sqrt(out)

sqrt(out)/sqrt(nrow(cces))
sqrt(out)/sqrt(length(Y))
length(weights)
