---
title: "A Look at the Weights: NO PID"
output:
...
#weights weights LA here
# weights_LA <- pew %>% mutate(pew4_wt = wts$wt4,
#                              k_wt = wts$wtkbal_nopid,
#                              k_mf_wt = wts$wtkbal_mf_nopid)


#load("weights_dat.Rdata")
#rerunning as is no using nopid
load("weights_dat_nopid.Rdata")
kbal_wt_b.25x <- kbal_wt_b.25x_nopid
kbal_wt_mf <- kbal_wt_mf_nopid

#Look at: Age
mean_comp_age <- weights_LA %>% ungroup() %>%
...
check_bal <- function(variable, pew4 = pew_lwt_4, kbal = kbal_wt_b.25x, kbal_mf = kbal_wt_mf) {
    bal_p4 = as.matrix(svymean(~pew_lwt_4$variables[,variable], pew_lwt_4, na.rm = TRUE))
    bal_k = as.matrix(svymean(~kbal_wt_b.25x$variables[,variable], kbal_wt_b.25x, na.rm = TRUE))
    bal_kmf = as.matrix(svymean(~kbal_wt_mf$variables[,variable], kbal_wt_mf, na.rm = TRUE))
...
outcome_bal <- check_bal("recode_vote_2016")
female_bal <- check_bal("recode_female")
age_bal <- check_bal("recode_age_bucket")
interaction_bal <- check_bal("recode_race_educ_reg")
