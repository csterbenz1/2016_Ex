...
#                    linkernel = FALSE, scale = FALSE)
# K_large <- makeK(kbal_est$onehot_data, b=2*kbal_est$b, useasbases = c(rep(1, nrow(pew)), rep(0, nrow(cces))),
#                    linkernel = FALSE, scale = FALSE)
# K_wut <- makeK(kbal_est$onehot_data, b=kbal_est$b, useasbases = c(rep(1, nrow(pew)), rep(0, nrow(cces))),
#                    linkernel = FALSE, scale = FALSE)
# K_small<- makeK(kbal_est$onehot_data, b=0.5*kbal_est$b, useasbases = c(rep(1, nrow(pew)), rep(0, nrow(cces))),
#                    linkernel = FALSE, scale = FALSE)
# 
# kbal_est$K[1:3, 1:3]
...
# 
# K_large[1:3, 1:3]
# #see so multiplying by two we get back to the default
# exp(-raw_counts[1:3,1:3]*2/kbal_est$b)
# K_wut[1:3,1:3]
# #ok well def the kbal_est one def has larger var so it's reight
# #so we currently build the K internall with cat data where we multiply the b we pass out by 2
# var(as.vector(K_wut))
# var(as.vector(kbal_est$K))

#k so it's right and for the scree opklot lets do bmaxvar/2 bmaxvar*2
#that is 
kbal_est$K[1:3,1:3]
K_large <- makeK(kbal_est$onehot_data, b=4*kbal_est$b, useasbases = c(rep(1, nrow(pew)), rep(0, nrow(cces))),
                   linkernel = FALSE, scale = FALSE)
#the svd is the above with b=2*kbal_est$b
#then small is
K_small <- makeK(kbal_est$onehot_data, b=kbal_est$b, useasbases = c(rep(1, nrow(pew)), rep(0, nrow(cces))),
                   linkernel = FALSE, scale = FALSE)

K_v_small <- makeK(kbal_est$onehot_data, b=kbal_est$b*0.5, useasbases = c(rep(1, nrow(pew)), rep(0, nrow(cces))),
                   linkernel = FALSE, scale = FALSE)
...
K_small_svd <- svd(K_small)
K_large_svd <- svd(K_large)
K_v_small_svd <- svd(K_v_small)

ggdata <- data.frame(b = c( rep("2.33 = argmax V(K)*1/4", 2052),
                            rep("5.66 = argmax V(K)*1/2", 2052),
                            rep("11.3 = argmax V(K)", 2052),
                            rep("22.64 = argmax V(K)*2", 2052)),
                     d = c(K_v_small_svd$d/K_v_small_svd$d[1],
                           K_small_svd$d/K_small_svd$d[1],
                           kbal_est$svdK$d/kbal_est$svdK$d[1],
                           K_large_svd$d/ K_large_svd$d[1]),
                     index = rep(c(1:2052), 4))

ggplot(ggdata) +
  geom_line(aes(y = d, x = index, color = as.factor(b))) +
  xlim(0,50) +
 # ylim(0,1) +
  ylab("Singular Values (Scaled)") +
  xlab("Singular Vector") +
  scale_color_discrete(name = "b", breaks = c("2.33 = argmax V(K)*1/4",
                                              "5.66 = argmax V(K)*1/2",
                                              "11.3 = argmax V(K)", 
                                              "22.64 = argmax V(K)*2")) +
  theme_bw()
