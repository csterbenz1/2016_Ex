...
coefs
coefs[,1] = c(-2, #female dem not born
              2, #ind x female
              1, #rep x female
...
coefs_outcome = -coefs*4
y = cces_expanded %*% coefs_outcome  
y = y 
a = -.15
b = .075
try = 1/(1 + exp(-(a + b*y)))
set.seed(1383904)
xbeta_outcome = try + rnorm(nrow(cces_expanded), mean = 0, sd = sd(try)*1)
summary(xbeta_outcome)
while(check_outcome(xbeta_outcome)) {
