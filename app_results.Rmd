...
pew_nowt <- suppressMessages(svydesign(~1, data = pew))
...
main_res_plot <- function(b = "maxvar", 
   ...
   comp_df_diff <- comp_df_diff %>% 
    ...
    comp_df_diff_plot <- comp_df_diff %>% filter(!str_detect(source, "cces")) %>% 
        mutate(method =  case_when(str_detect(source, "unweighted") ~ "Raw Survey", 
                                   str_detect(source, "rake") ~ "Mean Calibration",
                                   str_detect(source, "post") ~ "Post-Stratification",
                                   str_detect(source, "kpop") ~ "Kpop", 
                                   TRUE ~ NA_character_ ))
    # blankrow <- data.frame(source = "test", est = 0, SE = 0, b = NA, 
    #                        MF = NA, aMF = NA, Conv = NA, 
    #               source_name = "test")
