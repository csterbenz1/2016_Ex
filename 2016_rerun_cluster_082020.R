...
#     model.matrix(as.formula("~."), .)
#
# kbal_data_nopid <- kbal_data_nopid[,-1]
...
#b=4
kbal_b4x_f <- kbal(allx=kbal_data,
...
save(kbal_b.125x_f,kbal_b.25x_f,kbal_b.5x_f,kbal_b1x_f,kbal_b2x_f,kbal_b4x_f, 
     file = paste0(path_save, "./mf_F_wPid_", 
...
#b=4
kbal_mf_b4x_f <- kbal(allx=kbal_data,
                      sampled = kbal_data_sampled,
                      b = 4 * ncol(kbal_data),
                      incrementby = 1,
                      fullSVD = TRUE,
...
save(kbal_mf_b.125x_f, kbal_mf_b.25x_f, kbal_mf_b.5x_f,kbal_mf_b1x_f,kbal_mf_b2x_f,
     file = paste0(path_save, "mf_T_multiF.Rdata"))

save(kbal_mf_b.125x_f,kbal_mf_b.25x_f,kbal_mf_b.5x_f,kbal_mf_b1x_f,kbal_mf_b2x_f,kbal_mf_b4x_f, 
...
save(bb_comp, bb_comp_mf,
     file = paste0(path_save, "biasbound_08_18_runs.Rdata"))
...
kbal_b4x_s <- kbal_b4x_f[!(names(kbal_b4x_f) %in% c("K", "svdK"))]
...
kbal_mf_b4x_s <- kbal_mf_b4x_f[!(names(kbal_mf_b4x_f) %in% c("K", "svdK"))]
...
save( kbal_b4x_s,kbal_b2x_s,kbal_b1x_s,kbal_b.5x_s,kbal_b.25x_s,kbal_b.125x_s,
      kbal_mf_b4x_s,kbal_mf_b2x_s,kbal_mf_b1x_s,kbal_mf_b.5x_s, kbal_mf_b.25x_s, kbal_mf_b.125x_s,
      ...
      file =  paste0(path_save, "./wPid_all_runs_small_", 
             str_sub(gsub("[[:space:]|[:punct:]]", "_", gsub("[:alpha:]", "", Sys.time())), 
                     start = 1, end = -3), ".RData"))
...
kbal_wt_b4x <- svydesign(~1, data = pew,
                         weights = kbal_b4x_f$w[kbal_data_sampled ==1])
...
kbal_mf_wt_b4x <- svydesign(~1, data = pew,
                            weights = kbal_mf_b4x_f$w[kbal_data_sampled ==1])
...
comp_df <- data.frame(
    ...
    #with PID
    #b = 4
    Pew_kbal_b4x = svycontrast(svymean(~recode_vote_2016, kbal_wt_b4x,
                                       na.rm = TRUE), vote_contrast),
    Pew_kbal_mf_b4x = svycontrast(svymean(~recode_vote_2016, kbal_mf_wt_b4x,
                                          na.rm = TRUE), vote_contrast),
...
comp_df$b <- 99
comp_df[grep("b", levels(comp_df$source)), "b"] <- c(rep(4,2), 
                                                     rep(2,2), 
                                                     rep(1,2),
                                                     rep(.5, 2),
                                                     rep(.25,2),
...
#comp_df
#crudely add biasbound
#reorder so all regular then mf
comp_df <- comp_df[c(1,2,3,5,7,9,11,13,4,6,8,10,12,14),]
comp_df$biasbound <- 99
comp_df[-c(1,2), "biasbound"] <- c(bb_comp$bb_ratio, bb_comp_mf$bb_ratio)
comp_df$biasbound[1:2] <- "Not Weighted"


View(comp_df)

save(comp_df, file = paste0(path_save, "comp_df_08_18.Rdata"))
