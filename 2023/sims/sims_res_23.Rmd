---
title: "Kpop Simulation Results - 2023"
output: pdf_document
header-includes:
  - \usepackage{graphicx}
  - \usepackage{booktabs}
...
Even with the fairly limited complexity of the outcome and selection models, post-stratification is forced to drop a significant number of units, $\approx20%$, in the population due to empty cells/missing strata in the sample.
...
- **"post_stratification (reduc)"** not relevant any more to model (pid, gender, age (3way= X), race, region (X), income(3way = X) bornagain, edu x white (3way = X)
...
weights = lapply(sims,`[[`, 3)
b_vec = unique(weights %>% bind_rows() %>% select(b) %>% pull())
#length(b_vec)
