...
#yes: we dont hvae weights or 27 units because their strat dn exist in the population
sum(weights(post_strat_reduc))
length(weights(post_strat_reduc))
sum(weights(post_strat_reduc)/mean(sum(weights(post_strat_reduc))))
sum(weights(post_strat_reduc)*2052/sum(weights(post_strat_reduc)) )
sum(is.na(weights(post_strat_reduc)[1:2052]))
#why are there NAs....not good -> these are the "problem units" in pew not cces
sum(pew$strata_reduc %in% problem_reduc)


weights_bmaxvar <- data.frame(kpop_w = weights_cat[[1]]$kpop_w[c(1:nrow(pew))],
                              kpop_demos_w = weights_cat[[1]]$kpop_demos_w[c(1:nrow(pew))],
                              kpop_demos_edu_w = weights_cat[[1]]$kpop_demos_weduc_w[c(1:nrow(pew))],
                              kpop_all_w = weights_cat[[1]]$kpop_all_w[c(1:nrow(pew))],
                              rake_demos = weights(rake_demos_noeduc)[1:2052]/mean( weights(rake_demos_noeduc)[1:2052]),
                              rake_demos_wedu = weights(rake_demos_weduc)[1:2052]/mean(weights(rake_demos_weduc)[1:2052]),
                              rake_all = weights(rake_all_vars)[1:2052]/mean(weights(rake_demos_weduc)[1:2052]),
                              ps_reduced = (weights(post_strat_reduc)*2052/sum(weights(post_strat_reduc)))[1:2052] )

summary(weights_bmaxvar)
apply(weights_bmaxvar, 2, IQR)

#glimpse here: do more fully and cleanly below:
ggplot(weights_bmaxvar) + 
    geom_density(aes(x = kpop_w, color = "kpop"), alpha = .4) + 
    geom_density(aes(x = kpop_demos_w, color = "kpop + MF (Demos)"), alpha = .4) + 
    geom_density(aes(x = kpop_demos_edu_w, color = "kpop + MF (Demos w/Edu)")) + 
    geom_density(aes(x = kpop_all_w, color = "kpop + MF (All)")) + 
    #xlim(0,7) + 
    theme_bw()
...
#number of obsersvations to get to 90% sum
colSums(weights_bmaxvar)
#have to do ps manually because of the NAs
...
sorted <- cbind(sorted, c(sort(na.omit(weights_bmaxvar[,8]), decreasing = T), rep(NA, 27))   )


sum_90perc <- matrix(NA, ncol(sorted), nrow =1 )
for(j in 1:ncol(sorted)) {
    for(i in 1:nrow(sorted)) {
        if( sum(sorted[c(1:i), j]) >= .9*2052) {
            stop_index = i
            break
        }
    }
...
weights_gg <- c(weights_bmaxvar[,1], 
                    ...
                    weights_bmaxvar[,7], 
                    weights_bmaxvar[,8])
weights_gg <- data.frame(weight = as.numeric(weights_gg),
                         Method = c(rep("kpop", 2052),
                                    rep("kpop+MF (Demos)", 2052),
                                    rep("kpop+MF (D+Edu)", 2052),
                                    rep("kpop+MF (All)", 2052),
                                    rep("Mean Calibration (Demos)", 2052),
                                    rep("Mean Calibration (D+Edu)", 2052),
                                    rep("Mean Calibration (All)", 2052),
                                    rep("Post-Stratification (Reduced)", 2052)),
                         binary_method = c(rep("kpop", 4*2052),
                                           rep("Mean Calibration", 3*2052),
                                           rep("Post-Stratification", 2052)))
 
                         
weights_gg <- weights_gg %>% mutate(top_90  = c(rep(sorted[sum_90perc[1],1], 2052),
                                                rep(sorted[sum_90perc[2],2], 2052),
                                                rep(sorted[sum_90perc[3],3], 2052),
                                                rep(sorted[sum_90perc[4],4], 2052), 
                                                rep(sorted[sum_90perc[5],5], 2052), 
                                                rep(sorted[sum_90perc[6],6], 2052), 
                                                rep(sorted[sum_90perc[7],7], 2052),
                                                rep(sorted[sum_90perc[8],8], 2052)))
...
dist_w <- ggplot(weights_gg) + 
    ...
    theme_bw()
dist_w
#ggsave(dist_w, file = "/Users/Ciara/Dropbox/kpop/Updated/application/plots/dist_w_full.pdf", height = 6, width = 10)
#ggsave(dist_w, file = "./plots/dist_w_close.pdf", height = 6, width = 10)
...
### Margins for Various Weighting Schemes (cleaner version below)
...
allmargins <- function(pop_weights = TRUE,
    ...
    if(pop_weights & maxit == 500 & tolerance == 1e-06) {
        #NB!!! these are EXACTLY the same weights
        #I just reran with the updated lasso_061021.rds data files to ensure I definitely
        #did not mess up anything by using the more cleaned data
        #which i did not :) so these old weights are perfectly fine as well 
        ...
        # load(paste0(path_weights,"bmaxvar_cat_lasso061021_wTRUE_m500_t1e-06_2021-06-30.Rdata"))
        # cat("loading: bmaxvar_cat_lasso061021_wTRUE_m500_t1e-06_2021-06-30 \n")
        
        
        #PREVIOUS used maxvar with diagononal included 11.3133, 
        #these use without diagonal 11.3205, ever so slighly different (on default kpop mostly)
        load(paste0(path_weights,"bmaxvar_cat_nodiag_lasso061021_wTRUE_m500_t1e-06_2021-07-07.Rdata"))
        cat("loading: bmaxvar_cat_nodiag_lasso061021_wTRUE_m500_t1e-06_2021-07-07.Rdata \n")
    ...
    #all files prev to 7/07 uses these names
    # kpop_w <- weights_cat[[index]]$kpop_w
    # kpop_conv_w <- weights_cat[[index]]$kpop_w_conv
    # kpop_mf_w <- weights_cat[[index]]$kpop_mf_w
    # #kpop_demos_w <- weights_cat[[index]]$kpop_demos_w
    ...
    wmabserr$var[wmabserr$var == "educ_wh_3way"]<- "educ (3way) white"
    wmabserr_nooutcomes$var[wmabserr_nooutcomes$var == "income_5way"]<- "income (6way)"
    wmabserr_nooutcomes$var[wmabserr_nooutcomes$var == "relig_6way"]<- "relig (5way)"
    wmabserr_nooutcomes$var[wmabserr_nooutcomes$var == "educ_wh_3way"]<- "educ(3way) white"
...
cat_margins_popw_new <- allmargins()
View(cat_margins_popw$wmabserr)
View(cat_margins_popw_new$wmabserr)
cat_margins_popw_new$kable_err
cat_margins_popw$kable_outmom

cat_margins_popw_hmm <- allmargins(pop_weighted_avg = FALSE)
...
main_res_plot <- function(b = "maxvar", 
    ...
    if(pop_weights & maxit == 500 & tolerance == 1e-06) {
        ...
        #PREVIOUS used maxvar with diagononal included 11.32, 
        #these use without diagonal 11.31339, ever so slighly different (on default kpop mostly)
        load(paste0(path_weights,"bmaxvar_cat_nodiag_lasso061021_wTRUE_m500_t1e-06_2021-07-07.Rdata"))
        cat("loading: bmaxvar_cat_nodiag_lasso061021_wTRUE_m500_t1e-06_2021-07-07.Rdata \n")
...
#does not change w/ new no diag b
...
#ggsave("/Users/Ciara/Dropbox/kpop/Updated/application/plots/bmaxvar_votediff_targTrueVote_facet_8.5x4.5.pdf", width = 8.5, height = 4.5)
...
clean_results <- main_res_plot(b = "maxvar",
...
pew_out <- function(popw = TRUE,
    ...
    if(popw & tolerance == 1e-06 & maxit == 500) { 
           
            load(paste0(path_weights, "/allB_nodiag_cat_lasso061021_wTRUE_m500_t1e-06_2021-07-07.Rdata"))
            cat("loading:allB_nodiag_cat_lasso061021_wTRUE_m500_t1e-06_2021-07-07.Rdata")
                clean <- out_cat
                #b increasing by factors of 2
                weights <- weights_cat[3:13]
                #alternative b's: b mean and b_max
                weights_alt <- weights_cat[1:2]
    } else { 
        stop("weights not updated for these specifications \n")
    }  
    ...
    kbal_data_sampled <- c(rep(1, nrow(pew)), rep(0, nrow(cces)))
     ...
     kpop_mf_bmean <- svydesign(~1, data = pew, 
                             weights =
                                 weights_alt[[1]]$kpop_all_w[kbal_data_sampled==1])
     ...
     kpop_mf_bmaxvar <- svydesign(~1, data = pew, 
                             weights =
                                 weights_alt[[2]]$kpop_all_w[kbal_data_sampled==1])
    ...
    if(bigB) {
            ...
            kpop_mf_b128 <- svydesign(~1, data = pew, 
                                 weights =
                                     weights[[7]]$kpop_all_w[kbal_data_sampled==1])
            ...
            kpop_mf_b256 <- svydesign(~1, data = pew, 
                                 weights =
                                     weights[[8]]$kpop_all_w[kbal_data_sampled==1])
            ...
            kpop_mf_b512 <- svydesign(~1, data = pew, 
                                 weights =
                                     weights[[9]]$kpop_all_w[kbal_data_sampled==1])
            ...
            kpop_mf_b1024 <- svydesign(~1, data = pew, 
                                 weights =
                                     weights[[10]]$kpop_all_w[kbal_data_sampled==1])
            ...
            kpop_mf_b2048 <- svydesign(~1, data = pew, 
                                 weights =
                                     weights[[11]]$kpop_all_w[kbal_data_sampled==1])
    ...
    if(smallB) {
            ...
            kpop_mf_b64 <- svydesign(~1, data = pew, 
                                 weights =
                                     weights[[6]]$kpop_all_w[kbal_data_sampled==1])
            ...
            kpop_mf_b32 <- svydesign(~1, data = pew, 
                                     weights =
                                         weights[[5]]$kpop_all_w[kbal_data_sampled==1])
            kpop_mf_b16 <- svydesign(~1, data = pew, 
                                     weights =
                                         weights[[4]]$kpop_all_w[kbal_data_sampled==1])
            kpop_mf_b8 <- svydesign(~1, data = pew, 
                                    weights =
                                        weights[[3]]$kpop_all_w[kbal_data_sampled==1])
            kpop_mf_b4 <- svydesign(~1, data = pew, 
                                    weights =
                                        weights[[2]]$kpop_all_w[kbal_data_sampled==1])
            kpop_mf_b2 <- svydesign(~1, data = pew, 
                                    weights =
                                        weights[[1]]$kpop_all_w[kbal_data_sampled==1])
        }
    ...
    comp_df_plot_diff <-
        ...
        geom_pointrange(data = if(display_pew_mod) {comp_df_diff_merge  %>% filter(!str_detect(source, "cces"))} else {comp_df_diff %>% filter(!str_detect(source, "cces"))}) +
        scale_y_continuous(breaks = c(natl_diff*100, seq(-5, 10, 5)),
                           minor_breaks = NULL,
                           labels = scales::percent_format(scale=1, accuracy = 0.1)) +
        theme_bw() +
        ...
        theme(axis.text.x = element_text(angle = 70, hjust = 1,
                                         face=ifelse(comp_df_real$source_name[-1]  %in% 
                                                    ...
                                                    "bold","plain") )) +
        annotate(geom = "text", 
                 x =if(display_pew_mod) {nrow(comp_df_pR_merge)-1+.25} else{nrow(comp_df_pR)-1+.25}, 
...
pew_out <- function(popw = TRUE,
    ...
    comp_df_plot_real_margin <-
        ...
        aes(x = source_name, y = est, ymin = est - 1.96*SE, ymax = est + 1.96*SE,
            color = if(display_conv) {as.factor(Conv)} else {
                as.factor(method)
            }, shape= if(display_conv) {as.factor(MF)} else { NULL} ) +
    ...
    comp_df_plot_diff <-
        ...
        theme(axis.text.x = element_text(angle = 70, hjust = 1,
                                         face= ifelse(comp_df_diff$source_name[-1] %in% 
                                                        best_b,
                                                    "bold","plain") )) +
        annotate(geom = "text", 
...
#ggsave("/Users/Ciara/Documents/Cloud Documents/Hazlett:Hartman RA/Working/current plots/lasso_popw_votediff_t1e-06_m500_allb_8x6.pdf", width = 8, height = 6)
...
kpop_diag_new <- data.frame(L1_orig = out_cat$l1_orig, 
                        L1_opt = as.numeric(out_cat[, c("l1", "l1_demos", "l1_demos_weduc", 
                                             "l1_full")]), 
                        bb_orig = as.numeric(out_cat[, c("bb_orig", "bb_orig_demos", "bb_orig_demos_weduc", 
                                              "bb_orig_full")]),
                        bb_opt = as.numeric(out_cat[, c("bb_opt", "bb_opt_demos", "bb_opt_demos_weduc", 
                                              "bb_opt_full")])) %>%
    mutate(bbr = bb_orig/bb_opt)
                        
kpop_diag_new
rownames(kpop_diag_new) <- c("kpop", "kpop + MF (Demos)", "kpop + MF (D+Edu)", "kpop + MF (All)")

kable(kpop_diag_new,
      format = "latex",
      booktabs = T,
      col.names = c("L1 Original", "L1 Optimal", "Biasbound Original", "Biasbound Optimal", 
                    "Biasbound Ratio"),
      digits = 4,
