...
r5_nonlin_file = "/Users/Ciara_1/Documents/Cloud Documents/Hazlett:Hartman RA/2016 Election/2016_Ex/2023/sims/res_kpopTRUE_noise1_on2023-10-04_nsims1000.RData"
series = T
#load data
...
#two nas in rake_all and rake_demos_wedu
...
pass_sims = ifelse(series, nrow(est), length(good))
...
table_r5 = table_r5[,-1]
#drop mf all direct
if(!series) { table_r5 = table_r5[-which(rownames(table_r5) == "kpop+amf  (all)"),]}
#table_r5 = table_r5[-which(rownames(table_r5) == "kpop+mf  (all)"),]
# cat("Note that Raking on the True Model fails to converge ", sum(is.na(est$rake_truth))/nrow(est)*100, "percent of the time")

#SE coverage
if(!series) { sims = sims[good]}
...
```{r table_res_paper, eval =F, echo = F}
#moved main table to be just table_r5 so this is redundant
...
rnames= gsub("post-stratification: true selection model", "post-stratification", rnames)
rnames = gsub("amf", " mf", rnames)
...
```{r eval =F, include = F, warning = F, message= F, fig.width=8, fig.height=6}
...
```{r SEs, echo = F}
#clean_colnames = c("kpop", "kpop (conv)", "kpop+mf (all)", "kpop+mf (demos)", "kpop+mf (d+edu)", "kpop+mf (all prev)")
...
if(!series) {
    emp_SE_out= emp_SE_out[, !grepl( "conv|all",colnames(emp_SE_out))]
    warning("Check col order and colnames here~!", immediate. = T)
    emp_SE_out = emp_SE_out[, c(1,3,4,2)]
} else {
...
if(!series) {
    SE_coverage_out= SE_coverage_out[, !grepl( "conv|all",colnames(SE_coverage_out))]
    SE_coverage_out = SE_coverage_out[, c(1,3,4,2)]
    warning("Double check col order and colnames here~!", immediate. = T)
} else {
...
if(!series) {
    ...
    #this is ridiculously slow... R loops + df is so bad...
...
if(!series) {
    final_sum = final_sum[-which(final_sum$Estimator == "kpop+mf (all)"),]
    final_sum$Estimator[which(final_sum$Estimator == "kpop+amf")] = "kpop+mf (all)"
} else {
...
if(!series) {
    ...
    drop_methods = "conv|(all)"
    rownames(k_avg) = gsub("amf", "mf (all)", rownames(k_avg))
} else {
