...
r66_ps1.5_file = "~/Documents/Cloud Documents/Hazlett:Hartman RA/2016 Election/2016_Ex/Summer 2022/sims/noscale_kpopTRUE_noise0.707106781186548_on2022-12-22_nsims494.RData"

r66_ps_race_file = "~/Documents/Cloud Documents/Hazlett:Hartman RA/2016 Election/2016_Ex/Summer 2022/sims/noscale_res_kpopTRUE_noise0.707106781186548_on2022-12-23_nsims494.RData"
...
make_outcome <- function(noise, bern = F, 
    ...
    gg_p_include = ggplot(gg_dat) +
        ...
        ggtitle("Distribution of Seleciton Probabilities") +
        theme_bw()
    ...
    gg_p_include_pid = ggplot(gg_dat) +
        ...
        annotate(geom = "label", x=quantile(p_include,.75),
                 y=Inf, vjust = 1,
                 color = "red",
...
#r66_old =  make_outcome(noise = sqrt(2)*(1/2),  linear_model = F)
#plot(density(r33_lin$p_include))
# r33_lin$plot_pinclude
# r33_lin$plot_pinclude_pid
# r33_lin$plot_pinclude_outcome

load(r66_ps1.5_file)
r66_ps1.5  = make_outcome(noise = sqrt(2)*(1/2),
                          selection_model = as.formula(~recode_pid_3way +recode_female
                                                       + recode_age_bucket + recode_educ_3way
                                                       + recode_pid_3way:recode_age_bucket),
                          outcome_coefs = coefs_outcome,
                          selection_coefs = coefs,
                          linear_model = FALSE)
rm(coefs, coefs_outcome, outcome, sims)

load(r66_ps_race_file)
r66_ps_race = make_outcome(noise = sqrt(2)*(1/2), linear_model = F, 
                          selection_model = as.formula(~recode_pid_3way + recode_female
                                                       + recode_age_bucket + recode_educ_3way
                                                       + recode_race 
                                                       + recode_pid_3way:recode_age_bucket),
                          selection_coefs = coefs, 
                          p_include_denom = 12.5,
                          outcome_coefs = coefs_outcome)
...
## Model 1: No race
### Selection Model 1: (no race)
$$p(S=1) = logit^{-1}\Big( PID(3way) + Age(4way)+ Gender + Educ(3way) + PID(3way)*Age(4way)\Big)$$

Where coefs are chosen to be roughly realistic and yields a sample size around 1500 Namely:

```{r, echo = F}
r66_ps1.5$selection_coefs_kable %>%
  kable_styling(latex_options = "hold_position")
...
This yields the following sampling probabilities
```{r, echo = F}
r66_ps1.5$selection_prob_kable %>%
  kable_styling(latex_options = "hold_position")
...
Looking visually at the distribution of selection probabilities we have:

```{r, echo = F, fig.width=14, fig.height=16}
gridExtra::grid.arrange(r66_ps1.5$plot_pinclude, r66_ps1.5$plot_pinclude_pid, nrow=2)
```

\clearpage

### Outcome Model (no race)
Trying to keep things straight forward, I use the identical outcome model with coefficients chosen to roughly match the observed vote choice Democratic margin. In other words, again we have:

Model 1: 
$$p(Vote=D) = PID(3way) + Age(4way)+ Gender + Educ(3way) + PID(3way)*Age(4way)$$
Coefficients were chose to be realistic and to yield $\hat{y}$'s that lie within a probability range. This is done with an automated procedure yeilding the very specific values:

```{r, echo = F}
r66_ps1.5$coefs_outcome_kable %>%
  kable_styling(latex_options = "hold_position")
```

This yields a population target  in percentage points of:
```{r, echo = F}
cat(paste0("Linear=", r66_ps1.5$linear, " Target= ",round(r66_ps1.5$target,4)*100, "%"))
```


I then add normally distributed noise to this outcome with mean zero and standard deviation $\sigma = sd(Y)*noise$. I run simulations where this noise is $\frac{1}{2}*\sqrt(2)$ yielding an $R^2$ of .66. 

```{r, echo = F}
cat(paste0("For Non-Linear Model w/ R2=",round(r66_ps1.5$R2,3), " the correlation of Y with p(S=1) is ", round(r66_ps1.5$corr,2), "\n"))
...
cat(paste0("For Non-Linear Model w/ R2=",round(r66_ps1.5$R2,3), " PS must drop an average of ", round(mean(dropped_cells$sum),2),
...
kable(round(bias, 3), format = "latex", booktabs = T,
      caption = paste0("Linear=",outcome_obj$linear,"(no Race) Bias \\textbf{in Percent} across ", length(good),
                       " sims: All Methods (Target = ", 
                       round(mean(outcome),3)*100, "), $R^2$ on Outcome = ", round(outcome_obj$R2, 3))) %>%
  kable_styling(latex_options = "hold_position")

#extra column here but dont see what they are in the sims as all named methods accounted for

kable(round(r66_emp_SEs$emp_SEs[,grepl("kpop", colnames(r66_emp_SEs$emp_SEs))]*100,3), 
      format = "latex", booktabs = T, 
      caption = paste("Linear=",outcome_obj$linear,"(no Race) Empirical SE Results \\textbf{in Percent}: Avg SE + Boot SE (sd(estimate)) ", 
                          length(good), "sims; $R^2$ on Outcome = ", round(outcome_obj$R2, 3))) %>%
  kable_styling(latex_options = "hold_position")


kable(round(SE_coverage[-5,grepl("kpop", colnames(SE_coverage))], 3), format = "latex", booktabs = T,
          caption = paste("Linear=",outcome_obj$linear,"(no Race) SE Coverage Results: Kpop", 
                          length(good), "sims; $R^2$ on Outcome = ", round(outcome_obj$R2, 3))) %>%
  kable_styling(latex_options = "hold_position")
...
#mean(est$n)
rm(sims, outcome, bias, SE_coverage)
```
...
```{r, echo = F}
r66_ps_race$selection_coefs_kable %>%
  kable_styling(latex_options = "hold_position")
```

This yields the following sampling probabilities:
```{r, echo = F}
r66_ps_race$selection_prob_kable %>%
  kable_styling(latex_options = "hold_position")
...
```{r, echo = F, fig.width=14, fig.height=16}
gridExtra::grid.arrange(r66_ps_race$plot_pinclude, r66_ps_race$plot_pinclude_pid, nrow = 2)
```


### Outcome Model 2: adding Race
...
Coefficients again chosen through an automated procedure to be vaguely realistic and to yield $\hat{y}$'s that lie within a probability range:

```{r, echo = F}
r66_ps_race$coefs_outcome_kable %>%
  kable_styling(latex_options = "hold_position")
```

This yields a population target in percentage points of:
```{r, echo = F}
cat(paste0("Linear=", r66_ps_race$linear, " Target= ",round(r66_ps_race$target,4)*100, "%"))
```

I then add normally distributed noise to this outcome with mean zero and standard deviation $\sigma = sd(Y)*noise$. Again this noise is $\frac{1}{2}*\sqrt(2)$ yielding an $R^2$ of .66. 

```{r, echo = F}
cat(paste0("For Non-Linear Model w/ RACE included and R2=",round(r66_ps1.5$R2,3), "\n the correlation of Y with p(S=1) is ", round(r66_ps1.5$corr,2), "\n"))
...
cat(paste0("For Non-Linear Model w/RACE included and R2=",round(r66_ps_race$R2,3), " PS must drop an average of ", round(mean(dropped_cells$sum),2),
           "units\nw/a sd=", round(sd(dropped_cells$sum)),
           " which is ", round(100*(mean(dropped_cells$sum)/44932),2),"% of the population\n"))
rm(coefs, coefs_outcome, outcome, sims)
#cat(paste0("For R2=",round(r66$R2,3), " the correlation with p(S=1) is ", round(r66$corr,2), "\n"))
...
#### Visual Correlation of Selection and Outcome Model 2 with Race
...
## Results Model 2: with Race 

```{r non_lin_res_race, echo = F, warnings = F}
load(r66_ps_race_file)
outcome_obj = r66_ps_race
good = which(lapply(sims, function (x) return(class(x))) == "list")
...
table(samp_check$check.leq_1pp)
