...
if(!coverage_eval) {
    ...
    if(linear_model) {
        ...
        rownames(coefs) = colnames(cces_expanded)
        coefs
        # p = pew %>% group_by(recode_pid_3way, recode_age_bucket, recode_region) %>% count() %>%
        #     mutate(n = round(n/nrow(pew), 3))
        # c = cces %>% group_by(recode_pid_3way, recode_age_bucket, recode_region) %>% count() %>%
        #     mutate(n_c = round(n/nrow(cces), 3))
        #data.frame(p, c[,ncol(c)]) %>% arrange(n)
        #low populated groups are: rep x 18-35, rep 36-50, Dem x 18-35,  Dem 36-50, ind x18-35, ind 65+         #northeast x rep x18-35, northeast x rep 36-50, west x rep x 18-35      
        
        coefs[,1] = c(-4.4, #intercept -5 w race
                      .1, #selection of indep pos
                      .1, #selection of R pos
                      .1, #36-50,
                      .1, #51-64,
                      .2, #65+,
                       -.7, #male
        ...
        # survey_sim %>% group_by(recode_race) %>% count() %>% mutate(n = n/sum(sample))
        # cces %>% group_by(recode_race) %>% count() %>% mutate(n = n/nrow(cces))
        # cces %>% group_by(recode_race, recode_vote_2016) %>% count() %>% mutate(n = n/nrow(cces))
        # 
        if(length(unique(survey_sim$recode_race)) == 1) { 
            stop("only one cat of race included")
        }
        
        s = survey_sim %>% group_by(recode_pid_3way, recode_age_bucket) %>% count() %>%
            mutate(n = round(n/nrow(survey_sim), 3))
        c = cces %>% group_by(recode_pid_3way, recode_age_bucket) %>% count() %>%
            mutate(n_c = round(n/nrow(cces), 3))
        ...
        data.frame(s, c[,ncol(c)-1]) %>% mutate(diff = n_c - n) %>% arrange(n)
        #we've now dropped ind x 36-50 and rep 36-50 in the sample
        
        #################### DESIGN OUTCOME MODEL ##################
        coefs_outcome = coefs
        
        coefs_outcome[,1] = c(4.4, #intercept #5 w race
                                          -.05, #selection of indep pos
                                          -.08, #selection of R pos
                                          -.08, #36-50,
                                          .01, #51-64,
                                          -.2, #65+, 
        ...
        if(min(xbeta_outcome) <0 | max(xbeta_outcome) > 1) {warning("Outcome beyond prob support for some units when noise is added",
                                                                    immediate. = T)}
        summary(xbeta_outcome)
        # cor(xbeta_outcome, p_include)
        cces[which(xbeta_outcome == min(xbeta_outcome)),
...
system.time({
  sims <- mclapply(1:nsims, function(nsim) {
    ...
    if(eval_kpop) {
      ...
      out$dropped_cells = list(dropped_cells = dropped_cells,
...
nas = which(is.na(est[, "rake_truth"]))
length(nas)/length(good)
#subset kpop to only where rake truth did not fail (non zeros)
non_nas = est[-nas, cols]

bias = colMeans(non_nas)
bias = bias - mean(outcome)
bias = data.frame(bias = t(t(bias))*100)
bias = bias %>% arrange(desc(abs(bias)))
round(bias,3)
...
empirical_SEs <- function(sims, eval_kpop = T) {
    SEs = lapply(sims, `[[`, 2) %>% bind_rows()
    est <- lapply(sims, `[[`, 1) %>% bind_rows()
    cols = if(eval_kpop) { c(3:7,10:12,14:19)} else {c(3:7,10:12)}
    est =est[, cols]
...
empirical_SEs(sims = sims, eval_kpop = eval_kpop)
...
if(SAVE) {
  save(sims, outcome, SE_coverage, bias, table, plot_lasso_margin ,noise, eval_kpop,
