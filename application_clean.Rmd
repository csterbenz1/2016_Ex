...
load("/Users/ekhartman/Documents/GitHub/2016_Ex/cleaned data/Full SVD/weights_wPid_full.Rdata")
...
round(cbind(cces = svymean(formula_rake_demos_weduc, cces_awt),
            ...
            kpop_mf = svymean(formula_rake_demos_weduc, kpop_mf)) * 100, 2)
```
...
comp_df <- data.frame(
    ...
    pivot_longer(cols = everything(),
                 ...
                 names_pattern = "(.*)\\.(.*)") %>%
    rename(est = nlcon) %>%
    mutate(err_target = est - target_margin,
...
comp_df %>%
    ...
    annotate(geom = "text", x = 8.65, y = natl_margin, label = " True\n National\n Margin", hjust = 0, angle = -90, color = "gray") +
    annotate(geom = "text", x = 8.55, y = comp_df$est[comp_df$source == "cces"], label = " CCES Estimated\n Margin", hjust = 0, angle = 90) +
    theme(legend.title = element_blank()) +
  scale_x_discrete(limits=comp_df$source_name, expand = expansion(0, 1.1))
```

```{r save_plot}
ggsave("./plots/weighted_pew_results.pdf", width = 9, heigh = 4)
```