...
#note that chr is supposed to be asked as a follow up to relig only if dont know or something else. but we have quite a few others mixed in
pew %>% filter(!is.na(chr)) %>% group_by(relig) %>% summarise(n())
...
pew <- pew %>% mutate(
    recode_relig = factor(case_when(
        ...
        #out of 31 of these: 8 said yes christ, 22 not, 1 refused
        ...
        #out of these 36: 24 said yes christian, 5 said not, 7 refused
        relig == "Don't Know/Refused (VOL.)" & chr == "Yes" ~ "Protestant",
        #leaves only 12 missing
        TRUE ~ NA_character_),
    ...
    recode_born = case_when(born == "Yes, would" ~ "Yes",
                            ...
                            born == "Don't know/Refused (VOL.)" ~ NA_character_,
    ...
    #leaving the 28 that response don't know as they are 
    ...
    recode_income = factor(case_when(
        ...
        #222 refused
        income == "[VOL. DO NOT READ] Don't know/Refused" ~ NA_character_,
        #there should be no NAs
        TRUE ~ NA_character_),
        levels = c("<10k", "10-20k", "20-30k", "30-40k", "40-50k", "50-100k", "100-150k",">150k")),
    recode_income_5way = factor(case_when(
        ...
        #there should be no NAs
        TRUE ~ NA_character_),
        levels = c("<20k", "20-50k", "50-100k", "100-150k",">150k")),
...
cces_2016 <- cces_2016 %>% mutate(
    ...
    recode_relig = factor(case_when(
        ...
        #NAs remain NAs 38
        TRUE ~ NA_character_),
        levels = c("Protestant", "Jewish", "Catholic", "Nothing in particular", "Agnostic", "Atheist", "Buddhist", "Muslim", "Mormon", "Hindu", "Orthodox", "Something else")),
    ...
    recode_income = factor(case_when(
        ...
        #4788 do not say
        faminc == "Prefer not to say" ~ NA_character_,
        #9 NAs
        TRUE ~ NA_character_),
        levels = c("<10k", "10-20k", "20-30k", "30-40k", "40-50k", "50-100k", "100-150k",">150k")),
