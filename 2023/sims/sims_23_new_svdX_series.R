...
check_sample_outcome <- function(sample, pop = cces, selection_model, interaction_cols, interaction_cols_2 = NULL) {
    ...
    samp_counts$diff_05 = abs( samp_counts$diff) >= .05
...
sum(p_include)
div = sum(p_include)/n_sample_set
#pS_denom = 32
    ...
    for(nsim in 1:nsims)  {
        ...
        system.time({
            ...
            if(class(rake_demos_noeduc_svyd)[1] == "try-error") {
                ...
                res_rake_demos_noeduc = data.frame(min =NA, 
                                                   perc_25 = NA, 
                                                   mean =NA,
                                                   perc_75 = NA,
                                                   var = NA)
...
sims = out
#ood = which(lapply(sims, function (x) return(class(x))) == "list")
#length(good)
...
if(SAVE) {
    save(sims, outcome, tolerance, maxit, increment, min_num_dims,
         max_num_dims, noise, R2_outcome, eval_kpop,
         coefs, coefs_outcome, selection_model, p_include, n_sample_set,
...
if(eval_kpop) { 
    weights <- sims[3]$weights #lapply(sims, `[[`, 3) %>% bind_rows() 
    residuals <- sims[4]$residuals #lapply(sims, `[[`, 4) %>% bind_rows()
    ps_dropped = sims[5] #lapply(sims, `[[`, 5) %>% bind_rows()
    samp_check = data.frame(sims[6]$sample) #lapply(sims, `[[`, 6) %>% bind_rows()
} else { 
    weights = NULL
    residuals <- sims[3] #%>% bind_rows()
    ps_dropped = sims[4] #%>% bind_rows()
    samp_check = data.frame(sims[5]$sample) #lapply(sims, `[[`, 5) %>% bind_rows()
}
...
all_SE_coverage <- function(sims, series = T, adjust_bias = FALSE, drop_NA = F, truth = NULL, methods = c("rake|kpop")) {
    if(!series ) {
...
empirical_SEs <- function(sims, eval_kpop = T, na_rm = F, series  = T) {
    if(!series) {
        SEs = lapply(sims, `[[`, 2) %>% bind_rows()
        est <- lapply(sims, `[[`, 1) %>% bind_rows()
    } else {
