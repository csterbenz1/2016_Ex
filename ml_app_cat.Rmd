...
###### XXX do I want to have diff with cces_on_pew as well for cces_xgb or as below just use cces_on_ccess for cces_xgb for both
    ...
    comp_df_plot_diff <-
        ...
        annotate(geom = "text", 
                 ...
                 y = if(popw) {natl_diff * 100} else {comp_df_diff$est[comp_df_diff$source_name == "CCES XGB"]},
                 label = if(popw) {" True\n National\n Diff"} else {""},
                 hjust = 0, angle = -90,
                 color = "gray60", size = 3) +
        annotate(geom = "text",
        ...
        annotate(geom = "text", 
                 ...
                 y = if(popw) {comp_df_margin$est[comp_df_margin$source_name == "CCES Orig"]} else { comp_df_diff$est[comp_df_diff$source_name == "CCES XGB"]},
                 label = if(popw){" Weighted\n CCES\n Target"} else {""},
                 color = "black",
                 hjust = 0, angle = -90, size = 3) +
        #for spacing
    ...
    comp_df_plot_margin <-
        ...
        theme(axis.text.x = element_text(angle = 70, hjust = 1,
         ...
         annotate(geom = "text", 
                 ...
                 y = if(popw) {natl_margin * 100} else {comp_df_margin$est[comp_df_margin$source_name == "CCES XGB"]},
                 label = if(popw) {" True\n National\n Margin"} else {""},
                 hjust = 0, angle = -90,
                 color = "gray60", size = 3) +
        annotate(geom = "text", 
        ...
        annotate(geom = "text", 
                 ...
                 y = if(popw) {comp_df_margin$est[comp_df_margin$source_name == "CCES Orig"]} else {comp_df_margin$est[comp_df_margin$source_name == "CCES XGB"]}, 
                 label = if(popw) {" Weighted\n CCES\n Target"} else {""},
                 color = "black",
                 hjust = 0, angle = -90, size = 3) +
        #for spacing
    ...
    comp_df_plot_both_pR <-
       ...
       # geom_pointrange(data = comp_df_pR_pewoutcome, alpha = 0.4) +
        ...
        annotate(geom = "text", 
                 ...
                 y = if(popw) {natl_Rep * 100} else {comp_df_pR$est[comp_df_pR$source_name == "CCES XGB"]},
                 label = if(popw) {" True\n National\n % Rep"} else {""},
                 hjust = 0, angle = +90,
                 color = "gray60", size = 3) +
        annotate(geom = "text",
                 x = if(display_pew_mod){nrow(comp_df_pR_merge)+1} else {nrow(comp_df_pR)+1},
                 y = if(popw) {comp_df_pR$est[comp_df_pR$source_name == "CCES Orig"]} else {comp_df_pR$est[comp_df_pR$source_name == "CCES XGB"]},
                 label = if(popw) {" Weighted\n CCES\n Target"} else {""},
                 hjust = 0, angle = -90, size = 3) +
        annotate(geom = "text",
                 x = if(display_pew_mod){nrow(comp_df_pR_merge)+.55} else {nrow(comp_df_pR)+.55},
    ...
    comp_df_plot_both_pD <-
        ...
        annotate(geom = "text", 
                 x = if(display_pew_mod) {nrow(comp_df_pR_merge)+1}else {nrow(comp_df_pR)+1},
                 y = if(popw) {comp_df_pD$est[comp_df_pD$source_name == "CCES Orig"]} else { comp_df_pD$est[comp_df_pD$source_name == "CCES XGB"]},
                 label = if(popw) {" Weighted\n CCES\n Target"} else {""},
                 hjust = 0, angle = 90, size = 3) +
        annotate(geom = "text", 
                 x = if(display_pew_mod)  {nrow(comp_df_pR_merge)+.55}else {nrow(comp_df_pR)+.55},
