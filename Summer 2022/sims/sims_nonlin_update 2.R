...
check_sample_outcome <- function(sample, selection_model, interaction_cols) {
...
if(!coverage_eval) {
    ...
    if(linear_model) {
        ...
        coefs[,1] = c(-8, #intercept -5 w race
                      1, #selection of indep pos
        ...
        check_sample_outcome(survey_sim,selection_model, inter)$counts
        ...
        #dropped strata
        n_distinct(cces$strata) - n_distinct(survey_sim$strata)
        dropped_strata = unique(cces$strata)[which(!(unique(cces$strata) %in% unique(survey_sim$strata)))]
        cces %>% filter(strata %in% dropped_strata) %>% count()
...
# n_distinct(cces$strata) - n_distinct(survey_sim$strata)
# dropped_strata = unique(cces$strata)[which(!(unique(cces$strata) %in% unique(survey_sim$strata)))]
# cces %>% filter(strata %in% dropped_strata) %>% count()/nrow(cces)
...
system.time({
  sims <- mclapply(1:nsims, function(nsim) {
    ...
    check_2 = check_sample_outcome(survey_sim, selection_model, interaction_cols = inter)
    check_nums = c(leq_5pp = check_2$bad, leq_1pp = check_2$v_bad, fail = check_2$fail)
    ...
    if(eval_kpop) {
      ...
      kpop <- lapply(1:length(b_manual), function(i) {
        ...
        residuals = kpop_svyd_reduc$variables$outcome - predict(cv_fit$glmnet.fit, s = cv_fit$lambda.min, newx = x)
        ...
        names(kpop_reduc_se) = tryCatch(paste0("kpop_reduc_", names(kpop_reduc_se)), error = function(e) NA)
...
######## Bias
est <- lapply(sims, `[[`, 1) %>% bind_rows()
cols = if(eval_kpop) { c(3:7,10:12,14:ncol(est))} else {c(3:7,10:12)}
